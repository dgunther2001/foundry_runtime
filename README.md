...
* [SPSC Lock-Free Queue](#spsc-lock-free-queue)
    ...
    * [Performance Experiments](#spsc-queue-performance-experiments)
        ...
        * [4. Write vs Read Prefetch](#4-write-vs-read-prefetch)
...
### SPSC API
This is a single-producer, single-consumer lock-free queue implementation. It is implemented using an array of type `T` as a ring-buffer. It currently only works for trivially copyable types (proof of concept), but I am currently working on making it valid for all types. The API is quite simple and is intended to be used by two separate threads (consumer and producer). Only the producer may call `try_enqueue` and the consumer may call `try_dequeue`. We rely on this to avoid additional constraints associated with MPSC and MPMC queues. Violating this results in undefined behavior. Although the queue can be used entirely by a single thread, doing so would be pointless and likely be less peformant than an `std::queue`.   
...
At small sizes (~4-16 entries), the producer and consumer repeatedly operate on the same cachelines in tight sequence. Each enqueue requires exclusive line ownership that may be held shared by the consumer, triggering frequent ownership transitions. The memory footprint is confined to only a few cachelines, so these coherency transactions occur at a high rate throttling throughput. Additionally, due to the limited number of entries, the queue reaches full and empty states frequently. As a result, the producer and consumer spend a greater fraction of sim time in the failure path, which in this experiment simply yields to the OS. This further decreases throughput.   
...
```
On AArch64, clang lowers to `prfm` hint instructions which are non-binding hints that suggests the processor fetch the line before access. Worst case latency becomes more likely as reuse distance increases and the ring buffer footprint takes up larger amounts of the local cache:  
```
...
To mitigate this, we can use read and write prefetch (although I will show below that write prefetch does the heavy lifting for this type of workload). At small buffer sizes (this is independent of `sizeof(T)`), throughput is still dominated by ownership transitions and thread yielding. Prefetch does not solve this problem. However at larger buffer sizes, prefetch stablizes throughput of the local maximum achieved with `Pad=On Prefetch=Off`, allowing the queue to maintain performance across larger memory footprints. Prefetch distance did not significantly affect throughput for `uint64_t`, although I would expect excessive prefetch distances to not improve performace as significantly as reuse distance increases. I will explore the interaction between `prefetch_distance`, `sizeof(T)`, and `capacity` further below.  
...
After isolating producer (write) and consumer (read) prefetching, the results are unambiguous. Read prefetching only behaves identically to the no-prefetch curve, and write prefetching behaves identically to the full-prefetch curve. For this use case, write prefetch is clearly the lever that matters. Despite read prefetch not measurably affecting throughput in my experiments, I left it enabled for the prefetch option. This data strongly suggests that the throughput collapse without prefetch optimizations is dominated by L2/L3 RFO transactions. When the producer tries to write to a cacheline that it doesn't own exclusive, the core's L1d issues an RFO, invalidates copies on all other cores, and may have to get the line from higher cache levels. Alternatively, getting a shared copy of the line is comparably inexpensive and negligible compared to RFO latency. At higher memory footprints, the penalty of repeatedly fetching a line exclusive becomes the primary bottleneck. Write prefetch mitigates this by hinting the CPU to take ownership of the line and increasing the probability it is in the L1d before it is written to, reducing the cost of an L2/L3 RFO. The plateau achieved with write prefetch represents a rough performance ceiling once RFO is removed as the primary limiter.  
